...
def _get_transform_train():
    ...
    if args.noise is None or args.noise_value == 0:
        transforms.append(
            RandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5)
        )
    elif args.noise == "SHIFT":
        transforms.append(
            AlbuRandomCrop_shift(
                args.patch_size_train,
                shift_value=args.noise_value,
                scales=[1, args.scale],
                margin=0.5,
            )
        )
        ...
        transforms.append(
            AlbuRandomCrop_distort(
                args.patch_size_train,
                distort_value=args.noise_value,
                scales=[1, args.scale],
                margin=0.5,
...
def _get_Albu_transform_train():
    ...
    if args.noise is None:
        transforms.append(
            AlbuRandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5)
        )
    elif args.noise == "SHIFT":
        transforms.append(
            AlbuRandomCrop_shift(
...
def _get_Albu_transform_val():
    ...
    if not args.validation_only:
        ...
        transforms.append(
            AlbuCenterCrop(
                args.patch_size_val, allow_smaller=True, scales=[1, args.scale]
            )
        )
    else:
...
def get_datasets():
    if args.images is not None:
        dataset_train = _names_to_dataset(
            args.dataset_train, "traonly", transform=_get_transform_train()  # train
        )
        ...
        dataset_traval = _names_to_dataset(
            args.dataset_traval, "traval", transform=_get_transform_traval()
        )
    ...
    if not args.validation_only:
        ...
        dataset_traval = _names_to_dataset(
            args.dataset_val, "traval", transform=_get_transform_traval()
        )
        loader_traval = torch.utils.data.DataLoader(
            dataset_traval,
            batch_size=100,
