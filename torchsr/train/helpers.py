...
from losses import ContextualLoss, CoBiLoss

import itertools    ### SHKIM higher
...
def _name_to_dataset(name, split, transform):
    ...
    if name == DatasetType.AlignFormer:
        return AlignFormer(**kwargs)
    if name == DatasetType.RealSRv3:
        return RealSRv3(**kwargs)
    if name == DatasetType.ImagePairs:
        return ImagePairs(**kwargs)
    if name == DatasetType.Zoom:
        return Zoom(**kwargs)
...
def _get_transform_train():
    transforms = []
    if args.index_patch :
        transforms.append(Resize(960, scales=[1, args.scale]))
    if args.noise is None or args.noise_value == 0:
        transforms.append(RandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5))
    elif args.noise == 'SHIFT':
        transforms.append(AlbuRandomCrop_shift(args.patch_size_train, shift_value = args.noise_value, scales=[1, args.scale], margin=0.5))
        # transforms.append(RandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5))
    elif args.noise == 'ROTATE':
        transforms.append(RandomCrop_rotate(args.patch_size_train, rotate_value = args.noise_value, scales=[1, args.scale], margin=0.5))
    elif args.noise == 'DISTORT':
        transforms.append(AlbuRandomCrop_distort(args.patch_size_train, distort_value = args.noise_value, scales=[1, args.scale], margin=0.5, index_patch=args.index_patch))
#        transforms.append(RandomCrop_distort(args.patch_size_train, distort_value = args.distort, scales=[1, args.scale], margin=0.5))
    else:
        raise ValueError("args.noise has Unknown")
...
def _get_Albu_transform_train():
    transforms = []
    if args.noise is None :
        transforms.append(AlbuRandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5))
    elif args.noise == 'SHIFT':
        transforms.append(AlbuRandomCrop_shift(args.patch_size_train, shift_value = args.noise_value, scales=[1, args.scale], margin=0.5))
    elif args.noise == 'ROTATE':
        transforms.append(AlbuRandomCrop_rotate(args.patch_size_train, rotate_value = args.noise_value, scales=[1, args.scale], margin=0.5))
    elif args.noise == 'DISTORT':
        transforms.append(AlbuRandomCrop_distort(args.patch_size_train, distort_value = args.noise_value, scales=[1, args.scale], margin=0.5))
    else:
        raise ValueError("args.noise has Unknown")
    if DataAugmentationType.FlipTurn in args.augment:
        transforms.append(AlbuRandomFlipTurn())
    else:
        if DataAugmentationType.VFlip in args.augment:
            transforms.append(AlbuRandomVerticalFlip())
        if DataAugmentationType.HFlip in args.augment:
            transforms.append(AlbuRandomHorizontalFlip())
    transforms.append(AlbuToTensor())
    brightness = DataAugmentationType.BrightnessJitter in args.augment
    contrast = DataAugmentationType.ContrastJitter in args.augment
    saturation = DataAugmentationType.SaturationJitter in args.augment
    if brightness or contrast or saturation:
        transforms.append(AlbuColorJitter(
            brightness=0.2 if brightness else 0,
            contrast=0.1 if contrast else 0,
            saturation=0.1 if saturation else 0
        ))
    return AlbuCompose(transforms)


# def _get_transform_train_LR():
#     return _get_transform_train()

# def _get_transform_train_HR():
#     return _get_transform_train()

def _get_transform_traval():
    transforms = []
    # transforms.append(EdgeRemove(args.patch_size_traval, scales=[1, args.scale]))
    transforms.append(RandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5))
    if DataAugmentationType.FlipTurn in args.augment:
        transforms.append(RandomFlipTurn())
    else:
...
def _get_Albu_transform_traval():
    transforms = []
    transforms.append(AlbuRandomCrop(args.patch_size_traval, scales=[1, args.scale], margin=0.5))
    if DataAugmentationType.FlipTurn in args.augment:
        transforms.append(AlbuRandomFlipTurn())
...
def _get_Albu_transform_val():
    transforms = []
    if not args.validation_only:
        # Full images are too big: only validate on a centered patch
        transforms.append(AlbuCenterCrop(args.patch_size_val, allow_smaller=True, scales=[1, args.scale]))
    else:
        transforms.append(AlbuAdjustToScale(scales=[1, args.scale]))
    transforms.append(AlbuToTensor())
    return AlbuCompose(transforms)
...
def get_datasets():
    if args.images is not None:
        dataset_train = _names_to_dataset(args.dataset_train, 'traonly', #train
                                         transform=_get_transform_train())
        loader_train = torch.utils.data.DataLoader(
            dataset_train, batch_size=args.batch_size, shuffle=True,
            num_workers=args.workers, pin_memory=not args.cpu)
        dataset_traval = _names_to_dataset(args.dataset_traval, 'traval',
                                   transform=_get_transform_traval())
        loader_traval = torch.utils.data.DataLoader(
            dataset_traval, batch_size=100, shuffle=False,
            num_workers=args.workers, pin_memory=not args.cpu)
        loader_traval = itertools.cycle(loader_traval)
        return loader_train, None, loader_traval
    ...
    if not args.validation_only:
        ...
        loader_train = torch.utils.data.DataLoader(
            dataset_train, batch_size=args.batch_size, shuffle=True,
            num_workers=args.workers, pin_memory=not args.cpu)
        dataset_traval = _names_to_dataset(args.dataset_val, 'traval',
                                   transform=_get_transform_traval())
        loader_traval = torch.utils.data.DataLoader(
            dataset_traval, batch_size=100, shuffle=False,
            num_workers=args.workers, pin_memory=not args.cpu)
    ...
    return loader_train, loader_val, loader_traval
...
#            return torch.optim.Adam(model.params(), **kwargs)
...
def get_loss():
    ...
    if args.loss == LossType.CONTEXTUAL:
